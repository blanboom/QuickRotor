...
void nRF24_RXMode(uint8_t RX_PAYLOAD) {
	...
	nRF24_RWReg(nRF24_CMD_WREG | nRF24_REG_RF_CH,0x12); // Set frequency channel 110 (2.510MHz)
	nRF24_RWReg(nRF24_CMD_WREG | nRF24_REG_RX_PW_P0,RX_PAYLOAD); // Set RX payload length (10 bytes)
	nRF24_RWReg(nRF24_CMD_WREG | nRF24_REG_RF_SETUP,0x0F); // Setup
	nRF24_RWReg(nRF24_CMD_WREG | nRF24_REG_CONFIG,0x0B); // Config: CRC on (2 bytes), Power UP, RX/TX ctl = PRX
	CE_H();
	timer_sleep(1); // Hold CE high at least 10us
}
